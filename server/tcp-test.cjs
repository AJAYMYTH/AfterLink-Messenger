...
async function main() {
  ...
  // Reconnect with JWT for authenticated operations
  await test('Reconnect with JWT token', async () => {
    c1.close();
    c1 = new TcpTestClient(token);
    await c1.connect();
  });
  ...
  await test('React to message', async () => {
    const res = await c1.request('messages.react', {
      messageId, emoji: '\u{1F44D}', action: 'add',
    });
    if (!res.ok) throw new Error(JSON.stringify(res.error));
  });
  ...
  await test('Get message history', async () => {
    ...
    if (!res.ok) throw new Error(JSON.stringify(res.error));
    if (!Array.isArray(res.data) || res.data.length === 0) throw new Error('No messages');
  });
  ...
  await test('Get DM history', async () => {
    const res = await c1.request('dm.history', {
      otherUserId: user2.user.id, limit: 10,
    });
    if (!res.ok) throw new Error(JSON.stringify(res.error));
  });
  ...
  await test('Update presence', async () => {
    const res = await c1.request('presence.online', { status: 'dnd' });
    if (!res.ok) throw new Error(JSON.stringify(res.error));
  });
